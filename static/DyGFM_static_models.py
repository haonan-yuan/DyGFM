# DyGFM_static_models.py — static pre-training (GCN + contrastive + KL)

import torch
import torch.nn as nn
import torch.nn.functional as F

# --- GCN Layer Definition (from original code) ---
class GCN(nn.Module):
...
# --- InfoNCE loss function and auxiliary functions (unchanged) ---
...
class GaussianEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Enhance stability: clamp logvar
...
class JointContrastiveModel(nn.Module):
    ...
    def _encode(self, features, adj):
        ...
        # Pass GCN output to Gaussian encoder
    ...
    def forward(self, features_list, adj_list, neg_samples):
        """
        Forward pass, now returns contrastive loss and KL loss
        """
        ...
        # 1. Calculate InfoNCE contrastive loss (on sampled points z)
        contrastive_loss = compareloss(z_all, neg_samples, temperature=self.temperature)

        # 2. Calculate KL divergence loss (on distribution parameters mu and logvar)
