...
def generate_final_node_features(args, node_features, edge_features,
    ...
    adj = get_adj_tensor(args, num_nodes=static_node_features.shape[0], device=device)
    static_node_features = static_model.get_embeddings([static_node_features], [adj])
    dynamic_node_features = get_target_dynamic_embeddings(
        node_features, edge_features, source_nodes, dest_nodes, timestamps,
        ngh_finder, timer, args, device, dynamic_model,
    )
    new_node_features = (
...
def train_one_epoch(args, train_data, train_rand_sampler, train_ngh_finder, models, tokens, optimizers, criterion, device):
    ...
    if shot_size == 0:
        return 0, 0, 0
    train_indices = np.random.choice(len(train_data.sources), shot_size, replace=False)
    sources_batch, destinations_batch = train_data.sources[train_indices], train_data.destinations[train_indices]
...
def evaluate(args, eval_data, negative_sampler, full_ngh_finder, models, tokens, device, eval_type='Validation'):
    for model in models.values():
    ...
    with torch.no_grad():
        if eval_type in ['Test', 'Inductive']:
            ...
            ap, auc = eval_edge_prediction(
                ...
                device=device,
                edge_classifier=models['edge_classifier'],
                eval_mode='full',
            )
        else:
            shot_size = 30
        ...
        if eval_mode == 'full':
            ...
            for i in range(num_batch):
                ...
                pos_prob, neg_prob = compute_edge_probabilities(
                    ...
                    model_features, device, edge_classifier,
                )
...
def main(args):
    ...
    if node_features.shape[0] != num_nodes:
        raise ValueError(
            ...
            f"(ml_{args.dataset}_node.npy)."
        )
    max_node_id = int(max(full_data.sources.max(), full_data.destinations.max()))
    if max_node_id >= num_nodes:
        raise ValueError(
            f"Downstream max node id {max_node_id} >= num_nodes {num_nodes}."
        )
    validate_graph_alignment(args.dataset, num_nodes, load_static_edge_index(args.dataset))
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % args.val_freq == 0:
                ...
                val_ap, _ = evaluate(args, val_data, val_rand_sampler, full_ngh_finder, models, tokens, device, eval_type='Transductive')
                nn_val_ap, _ = evaluate(args, new_node_val_data, nn_val_rand_sampler, full_ngh_finder, models, tokens, device, eval_type='Inductive')
                should_stop, _ = early_stopper.early_stop_check(val_ap, edge_classifier)
                if should_stop:
                    print(f"Early stopping at epoch {epoch + 1}")
                    break
        print("\n--- Final Evaluation on Test Set ---")
        test_results = evaluate(args, test_data, test_rand_sampler, full_ngh_finder, models, tokens, device, eval_type='Test')
        nn_test_results = evaluate(args, new_node_test_data, nn_test_rand_sampler, full_ngh_finder, models, tokens, device, eval_type='Test')
        run_summary = {
