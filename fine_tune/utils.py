...
class EarlyStopMonitor(object):
    def __init__(self, max_round=3, higher_better=True, tolerance=1e-10, save_path='best_model.pth'):
    ...
    def early_stop_check(self, curr_val, model):
        ...
        self.epoch_count += 1
        if is_new_best:
            print(f"✅ New best score: {curr_val if self.higher_better else -curr_val:.4f}. Saving model to '{self.save_path}'")
            torch.save(model.state_dict(), self.save_path)
        return self.num_round >= self.max_round, is_new_best
